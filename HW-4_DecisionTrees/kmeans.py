...
def toyProblem():
  ...
  for i in range(1, 150):
    centroids, assignments, SSE = kMeansClustering(X, k=i, max_iters=20, visualize=False)
    SSE_vs_k.append(SSE)
...
def computeAssignments(dataset, centroids):
  ...
  for x in dataset:
    ...
    v = np.argmin(d)
    ret = np.append(ret, [v], axis=0)
  return np.ndarray.astype(ret, int)
...
def updateCentroids(dataset, centroids, assignments):
  ...
  for j in range(len(centroids)):
    I = assignments == j
    count = np.sum(I)
    s = np.sum(dataset * np.reshape(I, [I.shape[0],1]), axis=0) / (1 + count)
    if j == 0:
      nC = np.array([s])
    else:
      nC = np.append(nC, [s], axis=0)
    nCount = np.append(nCount, count)
  return nC, nCount
...
def calculateSSE(dataset, centroids, assignments):
  return np.sum(np.linalg.norm(dataset - centroids[assignments]))
